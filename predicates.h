...
__device__ REAL splitter; /* = 2^ceiling(p / 2) + 1.  Used to split floats in half. */
__device__ REAL epsilon; /* = 2^(-p).  Used to estimate roundoff errors. */
/* A set of coefficients used to calculate maximum roundoff errors.          */
__device__ REAL resulterrbound;
__device__ REAL ccwerrboundA, ccwerrboundB, ccwerrboundC;
__device__ REAL o3derrboundA, o3derrboundB, o3derrboundC;
__device__ REAL iccerrboundA, iccerrboundB, iccerrboundC;
__device__ REAL isperrboundA, isperrboundB, isperrboundC;
__device__ REAL ispwerrbound;
...
__device__ int grow_expansion //(elen, e, b, h)                /* e and h can be the same. */
...
__device__ int grow_expansion_zeroelim //(elen, e, b, h)       /* e and h can be the same. */
(int elen, REAL *e, REAL b, REAL *h) {
	REAL Q, hh;
	INEXACT REAL Qnew;
	int eindex, hindex;
...
__device__ int expansion_sum //(elen, e, flen, f, h)
/* e and h can be the same, but f and h cannot. */
(int elen, REAL *e, int flen, REAL *f, REAL *h) {
	REAL Q;
	INEXACT REAL Qnew;
...
__device__ int expansion_sum_zeroelim1 //(elen, e, flen, f, h)
/* e and h can be the same, but f and h cannot. */
(int elen, REAL *e, int flen, REAL *f, REAL *h) {
	REAL Q;
	INEXACT REAL Qnew;
...
__device__ int expansion_sum_zeroelim2 //(elen, e, flen, f, h)
/* e and h can be the same, but f and h cannot. */
(int elen, REAL *e, int flen, REAL *f, REAL *h) {
	REAL Q, hh;
	INEXACT REAL Qnew;
...
__device__ int fast_expansion_sum //(elen, e, flen, f, h)           /* h cannot be e or f. */
(int elen, REAL *e, int flen, REAL *f, REAL *h) {
	REAL Q;
	INEXACT REAL Qnew;
	INEXACT REAL bvirt;
...
__device__ int fast_expansion_sum_zeroelim //(elen, e, flen, f, h)  /* h cannot be e or f. */
(int elen, REAL *e, int flen, REAL *f, REAL *h) {
	REAL Q;
	INEXACT REAL Qnew;
	INEXACT REAL hh;
...
__device__ int linear_expansion_sum //(elen, e, flen, f, h)         /* h cannot be e or f. */
(int elen, REAL *e, int flen, REAL *f, REAL *h) {
	REAL Q, q;
	INEXACT REAL Qnew;
	INEXACT REAL R;
...
__device__ int linear_expansion_sum_zeroelim //(elen, e, flen, f, h)/* h cannot be e or f. */
(int elen, REAL *e, int flen, REAL *f, REAL *h) {
	REAL Q, q, hh;
	INEXACT REAL Qnew;
	INEXACT REAL R;
...
__device__ int scale_expansion //(elen, e, b, h)            /* e and h cannot be the same. */
(int elen, REAL *e, REAL b, REAL *h) {
	INEXACT REAL Q;
	INEXACT REAL sum;
...
__device__ int scale_expansion_zeroelim //(elen, e, b, h)   /* e and h cannot be the same. */
(int elen, REAL *e, REAL b, REAL *h) {
	INEXACT REAL Q, sum;
	REAL hh;
	INEXACT REAL product1;
...
__device__ int compress //(elen, e, h)                         /* e and h may be the same. */
(int elen, REAL *e, REAL *h) {
	REAL Q, q;
	INEXACT REAL Qnew;
	int eindex, hindex;
	INEXACT REAL bvirt;
...
__device__ REAL estimate //(elen, e)
(int elen, REAL *e) {
	REAL Q;
	int eindex;
...
__device__ REAL orient2dfast //(pa, pb, pc)
...
__device__ REAL orient2dexact //(pa, pb, pc)
(REAL *pa, REAL *pb, REAL *pc) {
...
__device__ REAL orient2dslow //(pa, pb, pc)
(REAL *pa, REAL *pb, REAL *pc) {
...
__device__ REAL orient2dadapt //(pa, pb, pc, detsum)
(REAL *pa, REAL *pb, REAL *pc, REAL detsum) {
	INEXACT REAL acx, acy, bcx, bcy;
...
__device__ REAL orient2d //(pa, pb, pc)
(REAL *pa, REAL *pb, REAL *pc) {
...
__device__ REAL orient3dfast //(pa, pb, pc, pd)
...
__device__ REAL orient3dexact //(pa, pb, pc, pd)
(REAL *pa, REAL *pb, REAL *pc, REAL *pd) {
...
__device__ REAL orient3dslow //(pa, pb, pc, pd)
(REAL *pa, REAL *pb, REAL *pc, REAL *pd) {
...
__device__ REAL orient3dadapt(REAL *pa, REAL *pb, REAL *pc, REAL *pd,
		REAL permanent) {
...
__device__ REAL orient3d(REAL *pa, REAL *pb, REAL *pc, REAL *pd) {
	...
	if ((det > errbound) || (-det > errbound)) {
		...
		//printf("Orient: More accuracy needed.\n");
	}
	return det;
...
__device__ REAL incirclefast //(pa, pb, pc, pd)
(REAL *pa, REAL *pb, REAL *pc, REAL *pd) {
...
__device__ REAL incircleexact //(pa, pb, pc, pd)
(REAL *pa, REAL *pb, REAL *pc, REAL *pd) {
	INEXACT REAL axby1, bxcy1, cxdy1, dxay1, axcy1, bxdy1;
	INEXACT REAL bxay1, cxby1, dxcy1, axdy1, cxay1, dxby1;
	REAL axby0, bxcy0, cxdy0, dxay0, axcy0, bxdy0;
...
__device__ REAL incircleslow //(pa, pb, pc, pd)
(REAL *pa, REAL *pb, REAL *pc, REAL *pd) {
...
__device__ REAL incircleadapt //(pa, pb, pc, pd, permanent)
(REAL *pa, REAL *pb, REAL *pc, REAL *pd, REAL permanent) {
	INEXACT REAL adx, bdx, cdx, ady, bdy, cdy;
	REAL det, errbound;
...
__device__ REAL incircle //(pa, pb, pc, pd)
(REAL *pa, REAL *pb, REAL *pc, REAL *pd) {
...
__device__ REAL inspherefast //(pa, pb, pc, pd, pe)
...
__device__ REAL insphereexact_w(REAL *pa, REAL *pb, REAL *pc, REAL *pd,
		REAL *pe, REAL wa, REAL wb, REAL wc, REAL wd, REAL we) {
...
__device__ REAL insphereexact //(pa, pb, pc, pd, pe)
(REAL *pa, REAL *pb, REAL *pc, REAL *pd, REAL *pe) {
	INEXACT REAL axby1, bxcy1, cxdy1, dxey1, exay1;
	INEXACT REAL bxay1, cxby1, dxcy1, exdy1, axey1;
	INEXACT REAL axcy1, bxdy1, cxey1, dxay1, exby1;
...
__device__ REAL insphereslow //(pa, pb, pc, pd, pe)
(REAL *pa, REAL *pb, REAL *pc, REAL *pd, REAL *pe) {
...
__device__ REAL insphereadapt //(pa, pb, pc, pd, pe, permanent)
					...
					* ((cez * daeps + dez * aceps + aez * cdeps)
					...
					+ (dex * dex + dey * dey + dez * dez)
							* ((aez * bceps - bez * aceps + cez * abeps)
									+ (aeztail * bc3 - beztail * ac3
											+ ceztail * ab3)))
					- ((aex * aex + aey * aey + aez * aez)
							* ((bez * cdeps - cez * bdeps + dez * bceps)
									+ (beztail * cd3 - ceztail * bd3
											+ deztail * bc3))
					...
					+ 2.0
							* (((bex * bextail + bey * beytail + bez * beztail)
									* (cez * da3 + dez * ac3 + aez * cd3)
									+ (dex * dextail + dey * deytail
											+ dez * deztail)
											* (aez * bc3 - bez * ac3 + cez * ab3))
									- ((aex * aextail + aey * aeytail
											+ aez * aeztail)
...
__device__ REAL insphere_w(REAL *pa, REAL *pb, REAL *pc, REAL *pd, REAL *pe,
		REAL wa, REAL wb, REAL wc, REAL wd, REAL we) {
	REAL aex, bex, cex, dex;
	REAL aey, bey, cey, dey;
	REAL aez, bez, cez, dez;
	...
	if ((det > errbound) || (-det > errbound)) {
		...
		//printf("In sphere: More accuracy needed.\n");
	}
...
__device__ REAL orient3dexact_l_w(REAL *pa, REAL *pb, REAL *pc, REAL *pd,
		REAL wa, REAL wb, REAL wc, REAL wd, bool lifted) {
	...
	if (lifted) {
		...
		Two_Two_Diff(aax1, aax0, aay1, aay0, xxyy[3], xxyy[2], xxyy[1],
				xxyy[0]);
		Two_Product(pa[2], pa[2], aaz1, aaz0);
		Two_One_Diff(aaz1, aaz0, wa, zzw[2], zzw[1], zzw[0]);
		palen = fast_expansion_sum_zeroelim(4, xxyy, 3, zzw, palift);
		...
		pblen = fast_expansion_sum_zeroelim(4, xxyy, 3, zzw, pblift);
		...
		pclen = fast_expansion_sum_zeroelim(4, xxyy, 3, zzw, pclift);
		...
		pdlen = fast_expansion_sum_zeroelim(4, xxyy, 3, zzw, pdlift);
	} else {
		palen = 1;
		palift[0] = pa[2];
		pblen = 1;
	...
	templen = fast_expansion_sum_zeroelim(cdlen, cd, dalen, da, temp56);
	cdalen = fast_expansion_sum_zeroelim(templen, temp56, aclen, ac, cda);
	templen = fast_expansion_sum_zeroelim(dalen, da, ablen, ab, temp56);
	dablen = fast_expansion_sum_zeroelim(templen, temp56, bdlen, bd, dab);
	...
	templen = fast_expansion_sum_zeroelim(ablen, ab, bclen, bc, temp56);
	abclen = fast_expansion_sum_zeroelim(templen, temp56, aclen, ac, abc);
	templen = fast_expansion_sum_zeroelim(bclen, bc, cdlen, cd, temp56);
	bcdlen = fast_expansion_sum_zeroelim(templen, temp56, bdlen, bd, bcd);
...
__device__ REAL orient3D_l_w(REAL* p0, REAL* p1, REAL* p2, REAL* p3, REAL w0,
		REAL w1, REAL w2, REAL w3, bool lifted) {
	REAL det = orient3dexact_l_w(p0, p1, p2, p3, w0, w1, w2, w3, lifted);
	return det;
}

__device__ REAL orient2dexact_l_w(REAL *pa, REAL *pb, REAL *pc, REAL wa,
		REAL wb, REAL wc, bool lifted) {
...
__device__ REAL orient2D_l_w(REAL * p0, REAL* p1, REAL* p2, REAL w0, REAL w1,
		REAL w2, bool lifted) {
	REAL det = orient2dexact_l_w(p0, p1, p2, w0, w1, w2, lifted);
	return det;
}

__device__ REAL orient1dexact_l_w(REAL *pa, REAL *pb, REAL wa, REAL wb,
		bool lifted) {
	if (!lifted)
		return (pa[0] - pb[0]);
	...
	Two_Two_Diff(azaz1, azaz0, bzbz1, bzbz0, cterms[3], cterms[2], cterms[1],
			cterms[0]);

	Two_Diff(wa, wb, wterms[1], wterms[0]);

	ulength = fast_expansion_sum_zeroelim(4, aterms, 4, bterms, u);
	vlength = fast_expansion_sum_zeroelim(ulength, u, 4, cterms, v);
	wlength = fast_expansion_sum_zeroelim(vlength, v, 2, wterms, w);

	return w[wlength - 1];
}

__device__ REAL orient1D_l_w(REAL* p0, REAL* p1, REAL w0, REAL w1,
		bool lifted) {
	REAL det = orient1dexact_l_w(p0, p1, w0, w1, lifted);
	return det;
}

__device__ REAL flipOrient(REAL ord) {
...
__device__ REAL orientation4SoS_w(

REAL* p0, REAL* p1, REAL* p2, REAL* p3, REAL* p4, REAL w0, REAL w1, REAL w2,
		REAL w3, REAL w4, int pi0, int pi1, int pi2, int pi3, int pi4) {
	const int DIM = 4;
	const int NUM = DIM + 1;
	...
	REAL* pt4Arr[NUM] = { p0, p1, p2, p3, p4 };
	REAL w4Arr[NUM] = { w0, w1, w2, w3, w4 };
	REAL* ip[NUM];
	REAL iw[NUM];
	...
	while (0 == orient) {
		...
		case 31:
		case 34: {
			orient = orient3D_l_w(op[0], op[1], op[2], op[3], ow[0], ow[1],
					ow[2], ow[3], lifted);
		}
			break;
		...
		case 32:
		case 33: {
			orient = orient2D_l_w(op[0], op[1], op[2], ow[0], ow[1], ow[2],
					lifted);
		}
			break;
		...
		case 48:
			...
			// 1D orientation involving the lifted coordinate
		case 29: {
			orient = orient1D_l_w(op[0], op[1], ow[0], ow[1], lifted);
		}
			break;
